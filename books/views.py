...
def add_opinion(request, pk):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            zmienna=p_server.add_opinion(book_id,opinion,user_name)
            if (zmienna):
                template = get_template("page/add_opinion_succes.html")
            else:
                template = get_template("page/add_opinion_fail.html")  
            variables = RequestContext(request,{'form':form,'book':book})
            output = template.render(variables)
            return HttpResponse(output)    
    else:
...
def delete_opinion(request,pk):
    if request.method == 'POST':
        ok=p_server.delete_opinion(pk)
        template = get_template("page/del_opinion.html")
        books = Book.objects.all()  #TUTAJ!!
        categories = Category.objects.all()  #TUTAJ!!
        variables = RequestContext(request,{'categories':categories,'books':books,'ok':ok})                
        output = template.render(variables)            
        return HttpResponse(output)                     
    else:
        ok=p_server.delete_opinion(pk)
